...
def run_qwq_model(prompt):
    """
    Run the QwQ-32B model with the given prompt.
    ...
    model = AutoModelForCausalLM.from_pretrained(
        model_name,
        torch_dtype="auto",  # Use the appropriate dtype based on available hardware
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run the QwQ-32B model")
    parser.add_argument("--prompt", type=str, default="How many r's are in the word 'strawberry'?",
                        help="The prompt to send to the model")
    
    args = parser.parse_args()
    
    response = run_qwq_model(args.prompt)
    print("\nModel Response:")
    print(response) 